...
def main(pae_file_path, pdb_path, pae_cutoff, dist_cutoff):
...
#pae_AURKA_TPX2_model_0.npz
    ...
    def classify_chains(chains, residue_types):
        ...
        for chain in unique_chains:
            ...
            nuc_count = np.sum(np.isin(chain_residues, nuc_residue_set))
    ...
    for chain1 in unique_chains:
        for chain2 in unique_chains:
            ...
            for i in range(numres):
                ...
                # Write the per-residue results to the output dictionary
                results_dict_per_res['i'].append(i)
                results_dict_per_res['AlignChn'].append(chain1)
                results_dict_per_res['ScoredChain'].append(chain2)
                results_dict_per_res['AlignResNum'].append(residues[i]['resnum'])
                results_dict_per_res['AlignResType'].append(residues[i]['res'])
                results_dict_per_res['AlignRespLDDT'].append(plddt[i])
                results_dict_per_res['n0chn'].append(int(n0chn[chain1][chain2]))
                results_dict_per_res['n0dom'].append(int(n0dom[chain1][chain2]))
                results_dict_per_res['n0res'].append(int(n0res_byres[chain1][chain2][i]))
                results_dict_per_res['d0chn'].append(d0chn[chain1][chain2])
                results_dict_per_res['d0dom'].append(d0dom[chain1][chain2])
                results_dict_per_res['d0res'].append(d0res_byres[chain1][chain2][i])
                results_dict_per_res['ipTM_pae'].append(iptm_d0chn_byres[chain1][chain2][i])
                results_dict_per_res['ipSAE_d0chn'].append(ipsae_d0chn_byres[chain1][chain2][i])
                results_dict_per_res['ipSAE_d0dom'].append(ipsae_d0dom_byres[chain1][chain2][i])
                results_dict_per_res['ipSAE'].append(ipsae_d0res_byres[chain1][chain2][i])
    ...
    for pair in sorted(chainpairs):
        ...
        for pair in (pair1, pair2):
            ...
            results_dict['Chn1'].append(chain1)
            results_dict['Chn2'].append(chain2)
            results_dict['PAE_Dist'].append(pae_string)
            results_dict['Type'].append('asym')
            results_dict['ipSAE'].append(ipsae_d0res_asym[chain1][chain2])
            results_dict['ipSAE_d0chn'].append(ipsae_d0chn_asym[chain1][chain2])
            results_dict['ipSAE_d0dom'].append(ipsae_d0dom_asym[chain1][chain2])
            results_dict['ipTM_af'].append(iptm_af)
            results_dict['ipTM_d0chn'].append(iptm_d0chn_asym[chain1][chain2])
            results_dict['pDockQ'].append(pDockQ[chain1][chain2])
            results_dict['pDockQ2'].append(pDockQ2[chain1][chain2])
            results_dict['LIS'].append(LIS[chain1][chain2])
            results_dict['n0res'].append(int(n0res[chain1][chain2]))
            results_dict['n0chn'].append(int(n0chn[chain1][chain2]))
            results_dict['n0dom'].append(int(n0dom[chain1][chain2]))
            results_dict['d0res'].append(d0res[chain1][chain2])
            results_dict['d0chn'].append(d0chn[chain1][chain2])
            results_dict['d0dom'].append(d0dom[chain1][chain2])
            results_dict['nres1'].append(residues_1)
            results_dict['nres2'].append(residues_2)
            results_dict['dist1'].append(dist_residues_1)
            results_dict['dist2'].append(dist_residues_2)
            results_dict['Model'].append(pdb_stem)
            ...
            if chain1 > chain2:
                ...
                # Store also the results when computed from maximum values
                results_dict['Chn1'].append(chain1)
                results_dict['Chn2'].append(chain2)
                results_dict['PAE_Dist'].append(pae_string)
                results_dict['Type'].append('max')
                results_dict['ipSAE'].append(ipsae_d0res_max[chain1][chain2])
                results_dict['ipSAE_d0chn'].append(ipsae_d0chn_max[chain1][chain2])
                results_dict['ipSAE_d0dom'].append(ipsae_d0dom_max[chain1][chain2])
                results_dict['ipTM_af'].append(iptm_af_value)
                results_dict['ipTM_d0chn'].append(iptm_d0chn_max[chain1][chain2])
                results_dict['pDockQ'].append(pDockQ[chain1][chain2])
                results_dict['pDockQ2'].append(pDockQ2_value)
                results_dict['LIS'].append(LIS_Score)
                results_dict['n0res'].append(int(n0res_max[chain1][chain2]))
                results_dict['n0chn'].append(int(n0chn[chain1][chain2]))
                results_dict['n0dom'].append(int(n0dom_max[chain1][chain2]))
                results_dict['d0res'].append(d0res_max[chain1][chain2])
                results_dict['d0chn'].append(d0chn[chain1][chain2])
                results_dict['d0dom'].append(d0dom_max[chain1][chain2])
                results_dict['nres1'].append(residues_1)
                results_dict['nres2'].append(residues_2)
                results_dict['dist1'].append(dist_residues_1)
                results_dict['dist2'].append(dist_residues_2)
    ...
    return results_dict, results_dict_per_res